...
function paintToCanvas() {
  ...
  //if videoRatio > windowRatio then decrease the canvas height, else decrease the canvas width (to account for ratio);
  ...
  return setInterval(() => {
    //updates feed every 16ms (60 frames/second)
...
function redEffect(pixels) {
  for (let i = 0; i < pixels.data.length; i += 4) {
    //i+=4 since each pixel is represented with 4 elements rgba
